...
<head>
  ...
  <style>
    ...
    .table-container {
      ...
      overflow-x: hidden;             /* adicione esta */
...
@media (max-width: 768px) {
      ...
      .table-container {
    overflow-x: hidden;                 /* permite rolar horizontalmente */
  ...
  .table {
    display: block;
    width: max-content;   /* ocupa a largura mínima necessária */
    min-width: 100%;      /* garante que nunca fique menor que a tela */
  }
  .table strong{
...
<body>
  ...
  if (form[f]) form[f].value = c[f] ?? '';
    ...
    async function saveCliente() {
      ...
      fields.forEach(f => {
  if (form[f]) {
    body[f] = (f === 'contribuinte') ? Number(form[f].value) : form[f].value;
  }
});
...
async function buscarCepCliente(cep) {
  ...
  try {
    ...
    // Dá foco no campo número (o ViaCEP não retorna número)
