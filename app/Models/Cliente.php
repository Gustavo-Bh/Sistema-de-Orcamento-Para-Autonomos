...
class Cliente
{
   ...
   public function getAllApp(?string $empresa_cnpj = null): array
{
    if ($empresa_cnpj) {
        $stmt = $this->db->prepare("
            SELECT id, nome, rua, numero, bairro, cep,
                   telefone, complemento, ie_rg, cpf_cnpj,
                   contribuinte, cidade_id, created_at, empresa_cnpj
            FROM clientes
            WHERE empresa_cnpj = ?
        ");
        $stmt->execute([$empresa_cnpj]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    return [];
}
